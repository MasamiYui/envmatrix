...
private final class LinkageDockerContextService: DockerContextService {
...
private final class LinkagePodmanContextService: PodmanContextService {
...
private final class LinkageDockerImageService: DockerImageService, @unchecked Sendable {
...
private final class LinkagePodmanImageService: PodmanImageService, @unchecked Sendable {
    func list() async throws -> [ContainerImage] { [] }
    func pull(reference: String, onLine: @Sendable @escaping (String) -> Void) throws -> StreamingHandle {
        throw ContainerContextsError.invalidInput("not used")
    }
...
private final class LinkageDockerContainerService: DockerContainerService, @unchecked Sendable {
...
private final class LinkagePodmanContainerService: PodmanContainerService, @unchecked Sendable {
    func list(all: Bool) async throws -> [ContainerInstance] { [] }
    func start(id: String) async throws {}
    func stop(id: String) async throws {}
    func restart(id: String) async throws {}
...
@MainActor
final class ContainerContextsViewModelLinkageTests: XCTestCase {
    private func makeViewModel() -> ContainerContextsViewModel {
        ContainerContextsViewModel(
            dockerService: LinkageDockerContextService(),
            podmanService: LinkagePodmanContextService(),
            dockerImageService: LinkageDockerImageService(),
            podmanImageService: LinkagePodmanImageService(),
            dockerContainerService: LinkageDockerContainerService(),
            podmanContainerService: LinkagePodmanContainerService()
        )
    }

    func test_useDocker_marksImagesAndInstancesStale() async {
        let vm = makeViewModel()
        XCTAssertFalse(vm.imagesVMDocker.isStale)
        XCTAssertFalse(vm.instancesVMDocker.isStale)

        await vm.useDocker("x")

        XCTAssertTrue(vm.imagesVMDocker.isStale)
        XCTAssertTrue(vm.instancesVMDocker.isStale)
    }

    func test_setPodmanDefault_marksPodmanSubVMs() async {
        let vm = makeViewModel()
