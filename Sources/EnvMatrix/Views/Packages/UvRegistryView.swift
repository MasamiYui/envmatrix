...
public struct UvRegistryView: View {
    @StateObject private var vm = UvRegistryViewModel()
    @State private var pendingPreset: UvRegistryPreset? = nil
    ...
    public var body: some View {
        ...
        .task { await vm.load() }
        .confirmationDialog(
            L("uvRepo.registry.confirmApply"),
            isPresented: Binding(
    ...
    private var header: some View {
        HStack(spacing: 8) {
            ...
            Text(L("uvRepo.registry.current"))
                .bold()
    ...
    private var presetsSection: some View {
        VStack(alignment: .leading, spacing: 8) {
            Text(L("uvRepo.registry.presets"))
                .font(.headline)
    ...
    private func presetCard(_ preset: UvRegistryPreset) -> some View {
        Button {
    ...
    private var customSection: some View {
        VStack(alignment: .leading, spacing: 8) {
            Text(L("uvRepo.registry.custom"))
                .font(.headline)
            HStack {
                TextField("https://…", text: $vm.customURL)
                    .textFieldStyle(.roundedBorder)
                Button(L("uvRepo.registry.apply")) {
