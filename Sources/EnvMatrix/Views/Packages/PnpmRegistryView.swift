...
public struct PnpmRegistryView: View {
    @StateObject private var vm = PnpmRegistryViewModel()
    @State private var pendingPreset: PnpmRegistryPreset? = nil
    ...
    public var body: some View {
        ...
        .task { await vm.load() }
        .confirmationDialog(
            L("pnpmRepo.registry.confirmApply"),
            isPresented: Binding(
    ...
    private var header: some View {
        HStack(spacing: 8) {
            ...
            Text(L("pnpmRepo.registry.current"))
                .bold()
    ...
    private var presetsSection: some View {
        VStack(alignment: .leading, spacing: 8) {
            Text(L("pnpmRepo.registry.presets"))
                .font(.headline)
    ...
    private func presetCard(_ preset: PnpmRegistryPreset) -> some View {
        Button {
            ...
            HStack(alignment: .top, spacing: 10) {
                ...
                Spacer(minLength: 0)
                if preset.url == vm.currentRegistry {
    ...
    private var customSection: some View {
        VStack(alignment: .leading, spacing: 8) {
            Text(L("pnpmRepo.registry.custom"))
                .font(.headline)
            HStack {
                TextField("https://…", text: $vm.customURL)
                    .textFieldStyle(.roundedBorder)
                Button(L("pnpmRepo.registry.apply")) {
